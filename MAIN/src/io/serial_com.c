...
int serial_set_mode(int fd, int databits, int parity, int stopbits)
{
    ...
    opts.c_cflag &= (~PARENB);
    if (parity != 0) opts.c_cflag |= PARENB;

    opts.c_cflag &= (~PARODD);
    if (parity == 1) opts.c_cflag |= PARODD;

    opts.c_cflag &= (~CSTOPB);
    if (stopbits == 2) opts.c_cflag |= CSTOPB;
...
int serial_set_baud(int fd, int baudrate)
{
    ...
    else
    {
        ...
        if (tcgetattr(fd, &tios)) perror("tcgetattr");

        cfsetispeed(&tios, B38400);
        cfsetospeed(&tios, B38400);
        tcflush(fd, TCIFLUSH);

        if (tcsetattr(fd, TCSANOW, &tios)) perror("tcsetattr");

        if (ioctl(fd, TIOCGSERIAL, &ser)) perror("ioctl TIOCGSERIAL");
        ...
        if (ioctl(fd, TIOCSSERIAL, &ser)) perror("ioctl TIOCSSERIAL");
#endif
    }
