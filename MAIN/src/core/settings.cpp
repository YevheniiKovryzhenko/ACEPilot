...
if(__parse_voltage_sensor_gen_settings(jobj, #name, &settings.name)){\
...
static int __parse_bool(json_object* jobj_str, const char* name, bool& value)
{
	struct json_object* tmp = NULL; // temp object
...
static int __parse_int(json_object* jobj_str, const char* name, int& value)
{
	struct json_object* tmp = NULL; // temp object
	if (json_object_object_get_ex(jobj_str, name, &tmp) == 0) {
		fprintf(stderr, "ERROR can't find %s in settings file\n", name);
...
static int __parse_int_positive(json_object* jobj_str, const char* name, int& value)
{
	int tmp;
	if (__parse_int(jobj_str, name, tmp))
	{
...
static int __parse_double(json_object* jobj_str, const char* name, double &value)
{
	struct json_object* tmp = NULL; // temp object
	if (json_object_object_get_ex(jobj_str, name, &tmp) == 0) {
		fprintf(stderr, "ERROR can't find %s in settings file\n", name);
...
static int __parse_double_min(json_object* jobj_str, const char* name, double& new_min, double min_min)
{
	double tmp1;
	if (__parse_double(jobj_str, name, tmp1))
	{
...
static int __parse_double_min_max(json_object* jobj_str, double& new_min, double& new_max)
{
	double tmp1, tmp2;
	if (__parse_double(jobj_str, "min", tmp1))
	{
		fprintf(stderr, "ERROR: failed to parse min\n");
		return -1;
	}
	if (__parse_double(jobj_str, "max", tmp2))
	{
...
static int __parse_double_positive(json_object* jobj_str, const char* name, double& value)
{
	double tmp;
	if (__parse_double(jobj_str, name, tmp))
	{
...
static int __parse_singal_filter_gen_type(json_object* in_json, const char* name, signal_filter_gen_type_t* type)
{
	...
	char* tmp_str = NULL;
	if (json_object_is_type(tmp_main_json, json_type_string) == 0) {
		fprintf(stderr, "ERROR: type should be a string\n");
		return -1;
	}
...
static int __parse_signal_filter_gen_settings(json_object* in_json, const char* name, signal_filter_gen_settings_t* filter)
{
	...
	// Parse filter type
	if (__parse_singal_filter_gen_type(tmp_main_json, "type", &filter->type) < 0)
	{
	...
	// Parse other entries
	if (__parse_double_positive(tmp_main_json, "dt", filter->dt))
	{
...
static int __parse_voltage_sensor_gen_settings(json_object* in_json, const char* name, voltage_sensor_settings_t* sensor)
{
	struct json_object* tmp_main_json = NULL;
	struct json_object* tmp = NULL;    // temp object
	...
	// Parse filter
	if (__parse_signal_filter_gen_settings(tmp_main_json, "filter", &sensor->filter) < 0)
	{
	...
	// Parse other entries
	if (__parse_bool(tmp_main_json, "en_warnings", sensor->en_warnings))
	{
		fprintf(stderr, "ERROR: failed to parse en_warnings flag for %s\n", name);
		return -1;
	}

	if (__parse_double_min(tmp_main_json, "nominal", sensor->nominal, 3.1))
	{
		fprintf(stderr, "ERROR: failed to parse nominal voltage for %s\n", name);
		return -1;
	}
	if (__parse_double_min(tmp_main_json, "min_critical", sensor->min_critical, 3.0))
	{
