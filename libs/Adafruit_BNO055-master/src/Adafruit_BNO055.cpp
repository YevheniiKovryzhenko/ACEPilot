 ...
 */
char Adafruit_BNO055::begin(adafruit_bno055_opmode_t mode, int32_t sensorID, int driver_bus_id, uint8_t devAddr) {

    if (i2c_dev.open(driver_bus_id, devAddr) < 0) {
        fprintf(stderr,"ERROR in begin: failed to open i2c port\n");
        return -1;
    }
    _sensorID = sensorID;
    ...
    if (id != BNO055_ID) {
        ...
        if (id != BNO055_ID) {
            fprintf(stderr, "ERROR in begin: can't find BNO55\n");
    ...
    if (setMode(OPERATION_MODE_CONFIG) < 0)
    {
        fprintf(stderr, "ERROR in begin: failed to set config mode\n");
        return -1;
    }

    /* Reset */
    if (write8(BNO055_SYS_TRIGGER_ADDR, 0x20) < 0)
    {
        fprintf(stderr, "ERROR in begin: failed to send reset request\n");
        return -1;
    }
    
    /* delay incrased to 30ms due to power issues https://tinyurl.com/y375z699 */
    /* have to increase even more (Jack) */
    delay(500);
    uint8_t tmp;
    ...
    if (write8(BNO055_PWR_MODE_ADDR, POWER_MODE_NORMAL) < 0)
    {
        fprintf(stderr, "ERROR in begin: failed to set to normal power mode\n");
        return -1;
    }    
    delay(10);
    ...
    */
    ...
    if (setMode(mode) < 0)
    {
        fprintf(stderr, "ERROR in begin: failed to set requested mode\n");
        return -1;
    }
    delay(20);
 ...
 *            OPERATION_MODE_NDOF]
 */
char Adafruit_BNO055::setMode(adafruit_bno055_opmode_t mode) {
    _mode = mode;
    char tmp = write8(BNO055_OPR_MODE_ADDR, _mode);
    if (tmp < 0)
    {
        fprintf(stderr, "ERROR in setMode: failed to write byte\n");
        return -1;
    }
    else
    {
 ...
 */
char Adafruit_BNO055::getVector(double* xyz, adafruit_vector_type_t vector_type) {
    uint8_t buffer[6];
    ...
    if (readLen((adafruit_bno055_reg_t)vector_type, buffer, 6) < 0)
    {
        return -1;
    }
    ...
    case VECTOR_GYROSCOPE:
        /* 1dps = 16 LSB */
        xyz[0] = ((double)x) / 16.0 * SENSORS_DPS_TO_RADS;
        xyz[1] = ((double)y) / 16.0 * SENSORS_DPS_TO_RADS;
        xyz[2] = ((double)z) / 16.0 * SENSORS_DPS_TO_RADS;
        break;
    case VECTOR_EULER:
        /* 1 degree = 16 LSB */
        xyz[0] = ((double)x) / 16.0 * DEG2RAD;
        xyz[1] = ((double)y) / 16.0 * DEG2RAD;
        xyz[2] = ((double)z) / 16.0 * DEG2RAD;
        break;
 ...
 */
char Adafruit_BNO055::getQuat(double* buff) {
    uint8_t buffer[8];
    //memset(buffer, 0, 8);
 ...
 */
/*
char Adafruit_BNO055::getEvent(sensors_event_t* event) {
    // Clear the event
    ...
    double tmp[3];
    getVector(tmp, VECTOR_EULER);
    event->orientation.x = tmp[0];
    event->orientation.y = tmp[1];
    event->orientation.z = tmp[2];
 ...
 */
/*
char Adafruit_BNO055::getEvent(double* buff,
    adafruit_vector_type_t vec_type) {
    getVector(buff, vec_type);
    return 0;
}
*/
 ...
 */
char Adafruit_BNO055::getSensorOffsets(uint8_t* calibData) {
    ...
    fprintf(stderr, "ERROR in getSensorOffsets: not fully calibrated\n");
    return -1;
}
 ...
 *  @return true if read is successful
 */
char Adafruit_BNO055::getSensorOffsets(
 ...
 */
int Adafruit_BNO055::write8(adafruit_bno055_reg_t reg, uint8_t value) {
    return i2c_dev.write_byte((uint8_t)reg, value);
}
...
uint8_t Adafruit_BNO055::read8(adafruit_bno055_reg_t reg) {
    uint8_t tmp;
    i2c_dev.read_byte((uint8_t)reg, &tmp);
    return tmp;
}
 ...
 */
int Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, uint8_t* buffer,
    uint8_t len) {
    return i2c_dev.read_bytes((uint8_t) reg, len, buffer);
}

uint64_t Adafruit_BNO055::millis(void)
{
    struct timespec ts;
    clock_gettime(CLOCK_BOOTTIME, &ts);
    return rc_timespec_to_millis(ts);
}

void Adafruit_BNO055::cleanup(void)
{
    i2c_dev.close();
}