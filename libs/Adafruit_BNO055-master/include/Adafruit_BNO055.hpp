...
public:
    ...
    char begin(adafruit_bno055_opmode_t mode = OPERATION_MODE_NDOF, int32_t sensorID = -1, int driver_bus_id = 1, uint8_t devAddr = DEF_I2C_ADDRESS);
    char setMode(adafruit_bno055_opmode_t mode);
    ...
    char getVector(double* xyz, adafruit_vector_type_t vector_type);
    char getQuat(double* buff);
    int8_t getTemp();

    /* Adafruit_Sensor implementation */
    //char getEvent(sensors_event_t*);
    //char getEvent(double* buff, adafruit_vector_type_t);
    //void getSensor(sensor_t*);

    /* Functions to deal with raw calibration data */
    char getSensorOffsets(uint8_t* calibData);
    char getSensorOffsets(adafruit_bno055_offsets_t& offsets_type);
...
private:
    ...
    int readLen(adafruit_bno055_reg_t, uint8_t* buffer, uint8_t len);
    int write8(adafruit_bno055_reg_t, uint8_t value);

    uint64_t millis(void); //for linux
